...
- 2026-09-23 - iPhone "still can't hear her": published site found to be running pre-a5156d2 code (never republished). Rebuilt the iPhone output on ElevenLabs' route (Web Audio -> MediaStream -> `<audio>`, mic live for the call, no audioSession writes), from their shipped SDK + live-site capture + WebKit source. Branch claude/iphone-audio-playback-compat-lmufp8, draft PR. Live AI routes failing since ~08:00 UTC (speech 502, turn error) - operator to check Lovable AI.
- 2026-09-26 - Operator confirmed on a real iPhone 13 that MARY is audible after PR #1 (the ElevenLabs-style `<audio>` element route, merged as 44318ba). The iPhone playback check that earlier entries called UNPROVEN is now proven on a device; the rest of the Apple audit (Mac Safari, no-mic path, AirPods) is still unproven.
- 2026-09-26 - Operator asked to finalize the Retell structure so the waitlist backend can switch to a Retell agent as soon as one exists. Built dormant behind `VOICE_PROVIDER` + `RETELL_API_KEY` / `RETELL_AGENT_ID` (`docs/architecture/retell-migration.md`, `retell/README.md`): `/api/voice`, the five `/api/retell/*` routes, the lazy browser adapter, and the agent as code in `retell/`; with nothing set the app is unchanged. This amends S4's "built later" for the code only; going live still waits on the operator's Retell account (voice id, webhook-badged key, D2 sheet) and the six operator questions in the doc.
