...
- The Retell agent is generated from `retell/` (`bun retell/setup.ts --site … --voice …` dry-runs into gitignored `retell/out/`). It edits a copy of `docs/mary-voice.md` in memory (seven `PLAYBOOK_EDITS`); the file itself is never changed, and `tests/unit/retell-config.test.ts` fails if a target stops occurring exactly once.
