...
export function buildPrompt(
  ...
  const driveRule = `\n\nYou drive this conversation. Unless the phase above is CLOSE, EXIT or the final callback turn, every turn must END on one concrete move — a question that comes out of their own last words, or a specific next step you are taking. Never end on a bare statement, an "okay", or a line that only summarises. Never ask permission questions or offer menus ("would you rather…", "shall I explain…", "want me to go on?", "does that make sense?", "any questions?") — decide, say the thing, then ask the question that gets you the next piece. The one exception is the WRAP phase: there, asking whether they have questions or want their spot finalised IS the concrete move. If they give you a flat answer ("okay", "sure", "hmm"), do not mirror it back: take the conversation somewhere with a specific, concrete question about their operation.`;
