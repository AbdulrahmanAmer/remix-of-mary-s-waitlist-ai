...
export function buildPrompt(
  ...
  const phase = callback
    ...
    : !introDone
      ? !history
        ? 'WELCOME — the very first thing you say. Greet them like a person first (a short hello on its own), then say who you are in one plain line, then what OmniSuite is in one plain line. Three short beats maximum, no stacking. End the intro by asking what you should call them, woven in naturally ("…and what should I call you?") — the name arrives as part of the hello, never as an intake question. Vary the wording; never use the same opener twice. Set introDone true.'
        : "WELCOME, RESUMED — your introduction was cut off before they heard all of it. React to what they just said first, warmly and briefly — never ignore it — then fold in the parts of the intro they have not heard yet (who you are, what OmniSuite is), without restarting from the top or repeating wording they already heard. End the turn by asking what you should call them, woven in naturally. Set introDone true only once the greeting, who you are and what OmniSuite is have all actually been said by the end of this turn."
      : !discoveryDone
